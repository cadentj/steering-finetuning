...
C=(sports pronouns 1)
D=(verbs sentiment 0)
E=(sports sentiment 1)
F=(sports verbs 1)
G=(pronouns sentiment 1)
H=(pronouns verbs 1)
    ...
    for split in A B C D E F G H; do
        ...
        case "$TYPE" in
            base)
                ...
                ;;
            interpreted)
                # intervention_path="/root/interpreted/${dataset_a}_${dataset_b}_features_intervention.pt"
                intervention_path="/root/interpreted/${dataset_b}_${dataset_a}_features_intervention.pt"

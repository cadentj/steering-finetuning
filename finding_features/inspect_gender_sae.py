...
def load_artifacts(model_id, sae_path):
    ...
    submodule_dict = {}
    for hookpoint, _ in latent_filter.items():
        layer_idx = int(hookpoint.split(".")[-1])
        sae = JumpReLUSAE(layer_idx).to(model.device).to(t.bfloat16)
        submodule_dict[hookpoint] = sae.encode

    return model, tokenizer, data, submodule_dict, latent_filter

model_id = "google/gemma-2-2b"
sae_path = "/root/layer_latent_map.json"

model, tokenizer, data, submodule_dict, latent_filter = load_artifacts(model_id, sae_path)
